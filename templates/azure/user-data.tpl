#cloud-config
bootcmd:
    - echo "PAGE_TITLE=Greetings from Azure!" | sudo tee -a /etc/environment
    - echo "HEADER_MESSAGE=Greetings from Azure!" | sudo tee -a /etc/environment
    - echo "SPECIAL_MESSAGE=${special_message}" | sudo tee -a /etc/environment
    - echo "HOST_NAME=$(hostname)" | sudo tee -a /etc/environment
    - echo "INTERNAL_IP_ADDRESS=$(ip addr show eth0 | awk '$1 == "inet" {gsub(/\/.*$/, "", $2); print $2}')" | sudo tee -a /etc/environment
